...
build_parameters <- function(p, percent_alpha, range_alpha, loc_scale = c(0,1), seed = NULL){
  ...
  varss <- rlogis(p)^2
  real.sigma <- force_symmetry( sqrt(diag(varss)) %*% real.theta %*% sqrt(diag(varss)) )
...
create_correlation_matrices <- function(real_corr, sample_size, df = 0, var_scale = NULL,
  ...
  pelet_matrices <- rWishart(sample_size, df, real_corr)/df

  return(pelet_matrices)
}

healthy <- create_correlation_matrices(real.sigma, healthy_N, Tlength)
sick <- create_correlation_matrices(real.sigma*alpha.mat, sick_N, Tlength)
...
vector_var_matrix_calc_COR <- function(MATR, nonpositive = c("Stop", "Force", "Ignore"),
  ...
  real.cov2 <- function(i, j, k, l) MATR[i,k]*MATR[j,l] + MATR[i,l]*MATR[j,k]
...
Pelet_IID$Estimates$theta %>% is.positive.definite()
(Pelet_IID$Estimates$theta*create_alpha_mat(Pelet_IID$Estimates$alpha)) %>% is.positive.definite()
