...
corrmat_covariance_from_dt <- function(dt, use_cpp = FALSE, est_n = FALSE, only_diag = TRUE, ncores = 1, verbose = FALSE){
  array_ <- copy(dt)
  if(class(array_) != 'array') array_ <- corrmat_covariance_array_from_dt(
    dt = array_, use_cpp = use_cpp,
    ncores = ncores, verbose = verbose)
  sigma <- calculate_mean_matrix(array_)
  if(est_n){
    est <- t(dt) %*% dt / nrow(dt)
...
sum_of_squares2 <- function(
  alpha, theta, sick_dt, inv_sigma_array,
  linkFun, dim_alpha = 1,
  reg_lambda = 0, reg_p = 2){
...
estimate_loop_2 <- function(
  ...
  g12_array <- corrmat_covariance_array_from_dt(sick_dt)
  g12_array_solve <- sapply(array_tree(g12_array, 3), solve, simplify = 'array')
  ...
  steps[[1]] <- list(
    theta = temp_theta,
    alpha = temp_alpha,
    value = sum_of_squares2(
      alpha = temp_alpha,
      theta = temp_theta,
      sick_dt = sick_dt,
      inv_sigma_array = g12_array_solve,
      linkFun = linkFun,
      dim_alpha = dim_alpha,
      reg_lambda = model_reg_config$lambda,
      reg_p = model_reg_config$lp
