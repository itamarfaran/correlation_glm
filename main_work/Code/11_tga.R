...
which.drop <- which(is.na(Real.dta$corrmats[1,,1]), arr.ind = TRUE)
p <- dim(Real.dta$corrmats)[1] - length(which.drop)

All.data <- array(dim = c(p, p, dim(Real.dta$corrmats)[3]))

#Clean data from unknown coloumns
for(i in 1:dim(All.data)[3]) All.data[,,i] <- force_symmetry(Real.dta$corrmats[-which.drop,-which.drop,i])
...
sick.Real_t <- All.data[,,Real.dta$TGA]
...
save(Pelet.Real, fisherMatrGrad, file = paste0("main_work/Data/Enviroments/TGArun_", Sys.Date(), ".RData"))

tt.hess <- Sys.time()
fisherMatrHess <- ComputeFisher(Pelet.Real, sampleData$sick, "Hess", linkFun = linkFun)  %>% regularizeMatrix()
tt.hess <- Sys.time() - tt.hess

save(Pelet.Real, fisherMatrGrad, fisherMatrHess, file = paste0("main_work/Data/Enviroments/TGArun_", Sys.Date(), ".RData"))
