...
for(i in 1:dim(healthy.Real)[3]) healthyResid <- c(healthyResid,
                                                   triangle_to_vector(healthy.Real[,,i]-Real.estimate$theta))

sickResid <- numeric()
for(i in 1:dim(sick.Real)[3]) sickResid <- c(sickResid,
                                        triangle_to_vector(sick.Real[,,i] -
                                                             Real.estimate$theta*create_alpha_mat(Real.estimate$alpha)))
...
```{r}
Estimate.Loop2 <- function(theta0, alpha0, Healthy.ARR, Sick.ARR, T_thresh, max.loop = 50, eps = 10^(-3), min_reps = 3, method = "Nelder-Mead",
                           comp_hess = TRUE){
...
Pelet_Cov <- Estimate.Loop2(theta0 = triangle_to_vector(Pelet_IID$Estimates$theta),
                            ...
                            Sick.ARR = sick, T_thresh = Tlength)
```
...
ggplot(Alpha.estimate, aes(x = Real, y = Estimate, color = Model)) + 
  geom_abline(slope = 1, intercept = 0, linetype = 2, color = "red", alpha = 0.7, size = 1) + 
  geom_point(alpha = 0.7, size = 2) + ggtitle("Alpha Estimate ~ Real")
...
Results
```{r}
build_hyp.test <- function(Estimate.Loop2_object, Real = NULL, test = c("lower", "upper", "two-sided"), qval = 0.05, method = "none", const = 1, effectiveN = NULL){
...
tmp <- build_hyp.test(Pelet_Cov, alpha, method = "holm", const = 1, effectiveN = Pelet_Cov$Est_N)
...
chisq <- -2*( minusloglik(theta = Pelet_Cov$theta, alpha = Pelet_Cov$alpha, healthy.data = healthy.data, sick.data = sick.data) -
               minusloglik(theta = Pelet_Cov$theta, alpha = rep(1, length(Pelet_Cov$alpha)), healthy.data = healthy.data, sick.data = sick.data) )
c("Chisq_val" = chisq, "DF" = length(Pelet_Cov$alpha), "Pval" = 1 - pchisq(chisq, length(Pelet_Cov$alpha)))

```


Build V matrix for misspecified model!!!!!!!!!
```{r}
tmpFun2 <- function(i, theta, alpha){
  tmpFun <- function(i) 
}
```
...
if(p <= 13){
  ...
  for (b in 1:B){
    ...
    res_misspecified <- Estimate.Loop(healthy, sick)
    res_specified <- Estimate.Loop2(theta0 = triangle_to_vector(res_misspecified$Estimates$theta),
                                    alpha0 = res_misspecified$Estimates$alpha,
                                    Healthy.ARR = healthy_tmp, Sick.ARR = sick_tmp, T_thresh = Tlength,
                                    comp_hess = FALSE)
    simuldat[[b]] <- list(healthy = healthy_tmp, sick = sick_tmp,
                          misspecified = res_misspecified, specified = res_specified)
